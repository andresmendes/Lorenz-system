...
set(gca,'xlim',[1.4*min(states(:,1)) 1.4*max(states(:,1))],'ylim',[1.1*min(states(:,2)) 1.1*max(states(:,2))],'zlim',[0 50])
set(gca,'xtick',[],'ytick',[],'ztick',[])
set(gca,'Color','k')

an = annotation('textbox', [0.32 0.9, 0.5, 0.1], 'string', 'Lorenz system','FitBoxToText','on');
an.FontName     = 'Verdana';
an.FontSize     = 36;
an.LineStyle    = 'none';
an.FontWeight   = 'Bold';
...
for i=1:length(time)
    ...
    % Trajectory
    plot3(states(1:j,1),states(1:j,2),states(1:j,3),'Color',color(6,:),'LineWidth',1.5)
    
    % Dot
    plot3(states(j,1),states(j,2),states(j,3),'o','MarkerFaceColor',color(4,:),'MarkerSize',15)
